...
abstract class RingBufferFields<E> extends RingBufferPad {
	...
	// Buffer数组起始基址，用于UNSAFE内存操作时进行寻址，注意这个偏移量加上了用于填充的BUFFER_PAD大小
	private static final long REF_ARRAY_BASE;
	// 数组引用每个引用占用的大小=2^REF_ELEMENT_SHIFT。计算出来的相对位移数，比如对象引用大小是4byte，那么REF_ELEMENT_SHIFT=2，因为2的2次方=4；
 ...
 * Disruptor最主要的组件，责存储和更新事件对象。 如何往RingBuffer中发布事件： 1.定义好要生产的数据和相应的事件类(里面存放数据)。
...
public final class RingBuffer<E> extends RingBufferFields<E> implements Cursored, EventSequencer<E>, EventSink<E> {
	 ...
	 * @param sequencer    sequencer用于处理通过RingBuffer移动的事件的顺序
	 ...
	 * @param waitStrategy used to determine how to wait for new elements to become
	 *                     available.
	...
	/**
	 * 
	 * 使用默认等待策略{@link BlockingWaitStrategy}创建一个新的多生产者的RingBuffer。
	 *
	 * @param            <E> RingBuffer中的事件类
	 * @param factory    用于在RingBuffer内创建事件
	 * @param bufferSize 在RingBuffer中创建的元素数量
	 ...
	 * 
	 * 递增并返回环形缓冲区的下一个序列。 调用此方法应确保它们始终发布序列。 例如
	 * 
	 * <pre>
	...
	/**
	 * <p>
	 * 递增并返回RingBuffer的下一个序列。调用此方法应确保它们始终发布序列 例如：
	 * </p>
	 * 
	 * <pre>
	 * long sequence = ringBuffer.next();
	 ...
	 * </pre>
	 *
	 * 如果RingBuffer中没有可用空间，则此方法不会阻塞，而是抛出{@link InsufficientCapacityException}
	 * 
	 * @return 要发布到的下一个序列
	 ...
	 * @throws IllegalStateException 如果已经指定了gating sequences
	 */
	@Deprecated
	...
	/**
	 * 
	 * 从添加到RingBuffer的所有gating sequences中获取最小序列值。
	 *
	...
	/**
	 * 给定特定要求容量确定是否有可用空间量 注意，假设如果此方法不能返回true，则对{@link RingBuffer#next()}的调用将不会阻止。
	 * 设置此RingBuffer以处理多个生产者。
	 * 
	 * @param requiredCapacity 检查能力
	 * @return 如果特定的的需要容量是可用的
	 */
