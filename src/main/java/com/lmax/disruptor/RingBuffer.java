...
public final class RingBuffer<E> extends RingBufferFields<E> implements Cursored, EventSequencer<E>, EventSink<E>
{
     ...
     *        
     * 递增并返回环形缓冲区的下一个序列。 调用此方法应确保它们始终发布序列。
     * 例如
     * <pre>
    ...
    /**
     * <p>递增并返回RingBuffer的下一个序列。调用此方法应确保它们始终发布序列  例如：</p>
     * <pre>
     * long sequence = ringBuffer.next();
     * try {
     *     Event e = ringBuffer.get(sequence);
    ...
    /**
     * 将游标重置为特定值。这可以在任何时候使用，但值得注意的是，它可能导致数据竞争，并且只应在受控环境中使用。
     * 例如： 在初始化期间
     *
    ...
    /**
     * 给定特定要求容量确定是否有可用空间量
     * 注意，假设如果此方法不能返回true，则对{@link RingBuffer#next()}的调用将不会阻止。
