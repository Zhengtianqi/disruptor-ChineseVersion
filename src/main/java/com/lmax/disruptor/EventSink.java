...
public interface EventSink<E>
{
	 ...
	 * @param translator 用户指定的事件的翻译
	 * @param arg0       用户提供的参数。
	 * @return 如果值已发布为true; 如果容量不足为false
	 ...
	 * @param arg1       用户提供的参数
	 * @param arg2       用户提供的参数
	 * @return 如果值已发布true; 如果容量不足为false
	 * @see #publishEvent(EventTranslator)
	 */
    ...
    /**
	 * <p>将多个事件发布到RingBuffer 它处理声明下一个序列，从RingBuffer获取当前（未初始化）事件并在转换后发布声明的序列。</p>
	 *
	 * <p>通过此调用，要插入RingBuffer的数据将是一个字段（显式或匿名捕获） 因此，对于要插入RingBuffer的每个值，此调用将需要转换器的实例。</p>
	 * 
    ...
    /**
	 * <p>将多个事件发布到环形缓冲区。 它处理声明下一个序列， 从RingBuffer获取当前（未初始化）事件并在转换后发布声明的序列</p>
	 * 
	 * <p>通过此调用，要插入RingBuffer的数据将是一个字段（显式或匿名捕获）, 因此，此调用将需要为要插入RingBuffer的每个值的转换器实例</p>
	 *
